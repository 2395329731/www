...
function gb2py($text, $exp = '') {
	if(!$text) return '';
	if(AJ_CHARSET != 'GBK') $text = convert($text, AJ_CHARSET, 'GBK');
	$data = array();
...
function save_local($content) {
	...
	foreach($matches[2] as $k=>$url) {
		if(is_crsf($url)) $content = str_replace($url, AJ_SKIN.'image/nopic.gif', $content);
	}
	...
	if(strpos($file, 'file/upload') === false) {//Remote
		if($AJ['ftp_remote'] && $AJ['remote_url']) {
			if(strpos($file, $AJ['remote_url']) !== false) {
				...
				if($AJ['ftp_save']) delete_upload(AJ_PATH.'file/upload/'.$file, $userid);
			}
		}
	} else {
