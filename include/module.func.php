...
//发放佣金
function amount_record($username, $kehu, $amount, $editor, $status, $note = '') {
	global $db, $AJ_TIME;
	if($username && $amount) {
		$r = $db->get_one("SELECT money FROM {$db->pre}member WHERE username='$username'");
		$kehu = addslashes(stripslashes(strip_tags($kehu)));
		$note = addslashes(stripslashes(strip_tags($note)));
		$db->query("INSERT INTO {$db->pre}finance_amount (username,kehu,amount,addtime,status,note,editor) VALUES ('$username','$kehu','$amount','$AJ_TIME','$status','$note','$editor')");
	}
}
