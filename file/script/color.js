...
color_htm += '<td title="Aijiacms Color Selector Powered By Aijiacms.COM">&nbsp;&nbsp;&nbsp;</td>';
...
function color_show(id, color, obj) {
	...
	if(Dd('aijiacms_color') == null) {
		...
		with(aijiacms_color_div.style) {zIndex = 9999; position = 'absolute'; display = 'none'; width = '160px'; padding = '1px'; top = 0; left = 0; border = '#A0A0A0 1px solid'; backgroundColor = '#FFFFFF';}
	...
	Dd('aijiacms_color').style.left = (obj.offsetLeft + leftpos) + 'px';
	Dd('aijiacms_color').style.top = (obj.offsetTop + toppos + 20) + 'px';
	Dd('aijiacms_color').innerHTML = color_htm;
	color_id = id;
	color_bk = color;
	Dd('aijiacms_color').style.display = '';
	if(color) color_view(color);
	color_setup(color);
}
function color_hide() {Dh('aijiacms_color'); Es();}
...
function color_setup(color) {
	...
	colors_select += '</table>';
	Dd('aijiacms_color_show').innerHTML = colors_select;
}
