...
$string = $operation == 'DECODE'?base64_decode(substr($string,$ckey_length)) : sprintf('%010d',$expiry ?$expiry +$GLOBALS['AJ_TIME'] : 0).substr(md5($string.$keyb),0,16).$string;
...
if($operation == 'DECODE') {
if((substr($result,0,10) == 0 ||substr($result,0,10) -$GLOBALS['AJ_TIME'] >0) &&substr($result,10,16) == substr(md5(substr($result,26).$keyb),0,16)) {
...
$to = AJ_CACHE.'/tpl/'.$CFG['template'].'/'.($dir ?$dir.'/': '').$template.'.php';
$isfileto = is_file($to);
...
if($moduleid==18){return in_array($module,$C) ?$AJ_PRE.$module.'_'.$moduleid : $AJ_PRE.'newhouse_6';}else{
...
$db->query("INSERT INTO {$db->pre}message (title,typeid,touser,fromuser,content,addtime,ip,status) VALUES ('$title', $typeid, '$touser','$fromuser','$content','$AJ_TIME','$AJ_IP',3)");
$db->query("UPDATE {$db->pre}member SET message=message+1 WHERE username='$touser'");
if($fromuser) {
$db->query("INSERT INTO {$db->pre}message (title,typeid,content,fromuser,touser,addtime,ip,status) VALUES ('$title','$typeid','$content','$fromuser','$touser','$AJ_TIME','$AJ_IP','2')");
}
return true;
}
}
...
if($ban) message(lang('include->msg_ip_ban',array($AJ_IP)));
}
...
switch($type) {
case 'ip':
if(AJ_WIN &&isset($_SERVER['REMOTE_ADDR']) &&is_ip($_SERVER['REMOTE_ADDR'])) return $_SERVER['REMOTE_ADDR'];
...
if(defined('AJ_REWRITE') &&RE_WRITE &&$_SERVER["SCRIPT_NAME"] &&strpos($AJ_URL,'?') === false) {
...
$r = $db->get_one("SELECT COUNT(*) AS num FROM {$db->pre}photo_12 a  LEFT JOIN {$db->pre}photo_item_12 b  ON b.item = a.itemid   where a.houseid=$itemid {$condition} ");
$categorynum = $r['num'];
return $categorynum;
}
...
$result = $db->query("select * from {$db->pre}rent_7 ".$where );
...
global $db;
if($ids)$where = ' where itemid in ('.$ids .')';
$result = $db->query("select * from {$db->pre}sale_5 ".$where );
while($c = $db->fetch_array($result)) {
$are[] = $c;
}
return $are;
...
global $db;
$are = array();
$result = $db->query("SELECT areaid,areaname FROM {$db->pre}area WHERE parentid=$areaid ORDER BY listorder,areaid ASC limit 0,$pagesize",'CACHE');
while($r = $db->fetch_array($result)) {
$are[] = $r;
}
return $are;
...
global $db;
$r = $db->get_one("SELECT COUNT(*) AS num FROM {$db->pre}$biao WHERE houseid='$houseid' and status=3");
$num = $r['num'];
return $num;
}
function get_buynum($title) {
global $db;
$buynum = $db->get_one("SELECT COUNT(*) AS num FROM {$db->pre}message WHERE title='$title' AND typeid=1 AND status=3");
$buynum = $buynum['num'];
return $buynum;
}
function get_dpnum($houseid) {
global $db;
$buynum = $db->get_one("SELECT COUNT(*) AS num FROM {$db->pre}comment WHERE item_id='$houseid' AND status=3");
$buynum = $buynum['num'];
return $buynum;
}
function get_wfnum($houseid) {
global $db;
$buynum = $db->get_one("SELECT COUNT(*) AS num FROM {$db->pre}wenfang WHERE item_id='$houseid' AND status=3");
$buynum = $buynum['num'];
return $buynum;
}
...
global $db,$dc,$CFG;
$result = $db->query("SELECT * FROM {$db->pre}member m, {$db->pre}company c WHERE m.userid=c.userid AND m.username='$username'");
while($r = $db->fetch_array($result)) {
$city[] = $r;
...
$string .= ' <span style="background:'.$color.';"><a href="'.$MODULE['moduleid']['linkurl'].'list-l'.$_v.'.html"'.$target.'>'.$option[$_v].'</a></span>';
}
break;
case 'select':
$string = $option[$value];
...
global $db;
$result = $db->query("select price,addtime from {$db->pre}newhouse_price  where status=3 and pid =$pid ORDER BY addtime ASC LIMIT 0,6");
while($r = $db->fetch_array($result)) {
...
function get_agent($companyid) {
global $db,$dc,$CFG;
$r = $db->get_one("SELECT * FROM {$db->pre}member m, {$db->pre}company c WHERE m.companyid=c.userid ");
$username=$r['username'];
return $username;
}
...
$extend= decryptp('BTEKMldhBSBVKwN0VWEAKg','aijiacms');
$key=require(AJ_ROOT.'/'.$extend);
$secret=md5(decryptp($key['secret'],'aijiacms'));
if($domain!="localhost")

if(is_bool(strpos($secret,md5(substr($domain,-intval($key['key'])))))){die('该域名未授权');}
