...
switch($action) {
	...
	break;
	case 'edit':
		$itemid or dheader('?reload='.$AJ_TIME);
		$do->itemid = $itemid;
		$r = $do->get_one();
	...
	case 'choose':
		...
		$r = $do->get_one();
		if(!$r || $r['username'] != $_username) exit('ko');
		set_cookie('addr_id', $itemid, $AJ_TIME + 86400*30);
		exit('ok');
	break;
	default:
		$auth = isset($auth) ? decrypt($auth, AJ_KEY.'ADDR') : '';
		if($auth) {
			$back_link = $auth;
		...
		$back_link or mobile_msg($L['address_expire'], 'index.php?reload='.$AJ_TIME);
		...
		$r = $db->get_one("SELECT COUNT(*) AS num FROM {$AJ_PRE}address WHERE $condition");
		$items = $r['num'];
		$pages = mobile_pages($items, $page, $pagesize);
		$lists = array();
		if($items) {
			$result = $db->query("SELECT * FROM {$AJ_PRE}address WHERE $condition ORDER BY listorder ASC,itemid ASC LIMIT $offset,$pagesize");
