...
$AJ['im_web'] or dheader('index.php?reload='.$AJ_TIME);
...
switch($action) {
	...
	case 'load':
						...
						if($time > $chatlast && $word) {
							...
							if(preg_match_all("/\:([0-9]{3,})\)/i", $word, $m)) {
								foreach($m[0] as $u) {
									...
									if(is_file(AJ_ROOT.'/'.$MOD['moduledir'].'/'.$f)) $word = str_replace($u, '<img src="'.$MOD['linkurl'].$f.'"/>', $word);
								}
							}
							$word = str_replace('"', '\"', $word);
		...
		exit(convert($json, AJ_CHARSET, 'UTF-8'));
	break;
	case 'list':
		$data = '';
		$new = 0;
		...
		while($r = $db->fetch_array($result)) {
			...
			$data .= '<a href="chat.php?chatid='.$r['chatid'].'&reload='.$AJ_TIME.'"><img src="'.useravatar($r['user']).'" width="40" height="40"/>';
			$data .= '<ul>';
		...
		exit(convert($data, AJ_CHARSET, 'UTF-8'));
	break;
	default:
		if(isset($touser)) {
