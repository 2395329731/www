...
switch($action) {
	case 'user':
		...
		$lists = array();
		if($items) {
			$result = $db->query("SELECT * FROM {$AJ_PRE}comment WHERE $condition ORDER BY itemid DESC LIMIT $offset,$pagesize");
	...
	case 'post':
		...
		if($MOD['comment_time']) {
			...
			if($r && $AJ_TIME - $r['addtime'] < $MOD['comment_time']) exit('fast');
		}
		$content = isset($content) ? convert(input_trim(nl2br($content)), 'UTF-8', AJ_CHARSET) : '';
	...
	default:
		if($EXT['comment_api']) {
			...
			$lists = array();
			$condition = "item_mid=$mid AND item_id=$itemid AND status=3";
			$r = $db->get_one("SELECT COUNT(*) AS num FROM {$AJ_PRE}comment WHERE {$condition}");
			$items = $r['num'];
			$pages = mobile_pages($items, $page, $pagesize);
			if($items) {
				$result = $db->query("SELECT * FROM {$AJ_PRE}comment WHERE {$condition} ORDER BY itemid ASC LIMIT $offset,$pagesize");
