...
$moduleid = $moduleid;
$file_ext = 'jpg';
$updir = '../file/upload/'.timetodate($AJ_TIME, $AJ['uploaddir']).'/';
$filename = date('His', $AJ_TIME).''.rand(10, 99).''.$_userid.'.'.$file_ext;
$target = $updir.$filename;
//file_put($target, $img);
file_put_contents($target, $img);
//$target = realpath($target); 
$upalbum = array('name'=>$filename,'type'=>'image/jpeg','tmp_name'=>$target,'error'=>'0','size'=>$imgsize);
$_FILES = array('upalbum'=>$upalbum);
//exit('{"status":0,"content":"'.$imgsize.'","url":"'.$target.'"}');
...
if($target) {
	...
	$img_w = $img_h = 0;
	if($do->saveto) {//exit('{"status":0,"content":"'.$imgsize.'","url":"ab"}');
	...
	exit('{"status":0,"content":"'.$imgsize.'","url":"'.$saveto.'"}');
	dalert('', '', $js);
} else {
