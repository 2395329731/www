...
switch($action) {
	case 'success':
		...
		$t = $db->get_one("SELECT email,mobile,vmobile,groupid FROM {$AJ_PRE}member WHERE userid='$userid'");
	...
	case 'send':
		...
		if(is_email($_SESSION['f_key'])) {
			...
			if($_SESSION['mobile_time'] && (($AJ_TIME - $_SESSION['mobile_time']) < 180)) exit('ko');
	...
	case 'verify':
		...
		(isset($_SESSION['f_uid']) && isset($_SESSION['f_key'])) or exit('ko');
		$userid = intval($_SESSION['f_uid']);
		$t = $db->get_one("SELECT email,vemail,mobile,vmobile,groupid FROM {$AJ_PRE}member WHERE userid='$userid'");
		$t or exit('ko');
		if($t['groupid'] == 2 || $t['groupid'] == 4) exit('ko');
		$vemail = $t['vemail'];
		if(is_email($_SESSION['f_key'])) {
		...
		$db->query("UPDATE {$AJ_PRE}member SET password='$pass',passsalt='$salt',vemail='$vemail' WHERE userid='$userid'");
		exit('ok');
	break;
	case 'check':
		...
		if($type == 'mobile') {
			...
			$t = $db->get_one("SELECT userid FROM {$AJ_PRE}member WHERE mobile='$mobile' AND vmobile=1");
			if($t) {
			...
			$t = $db->get_one("SELECT userid FROM {$AJ_PRE}member WHERE email='$email'");
			if($t) {
				$_SESSION['f_uid'] = $t['userid'];
	...
	break;
	case 'user':
		isset($type) or dheader('?reload='.$AJ_TIME);
		if($type == 'mobile') {
			$could_mobile or dheader('?reload='.$AJ_TIME);
			$head_name = $L['forgot_by_sms'];
		} else if($type == 'email') {
			$could_email or dheader('?reload='.$AJ_TIME);
