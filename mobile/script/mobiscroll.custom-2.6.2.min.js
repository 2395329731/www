...
function(a){var d=[],e;for(e=0;e<a;e++)d[e]=b.labels&&b.labels[e]?b.labels[e]:e;return d}(D),B=[],J=function(a){for(var b=[],d,e=!0,f=0;e;)if(d=t(a),b[f++]=d.key,e=d.children)a=d.children;return b}(u),J=j(J,D);a("#"+Q).remove();b.showInput&&(r=a('<input type="text" id="'+Q+'" value="" class="'+b.inputClass+'" placeholder="'+b.placeholder+'" readonly />').insertBefore(q),m.settings.anchor=r,b.showOnFocus&&r.focus(function(){m.show()}),b.showOnTap&&m.tap(r,function(){m.show()}));b.wheelArray||q.hide().closest(".ui-field-contain").trigger("create");
...
!1);e.invalid.length||(e.invalid=Q);e.group?e.rtl?(C=1,u=0):(C=0,u=1):(C=-1,u=0);a("#"+l).remove();B=a('<input type="text" id="'+l+'" class="'+e.inputClass+'" placeholder="'+e.placeholder+'" readonly />').insertBefore(j);a("option",j).each(function(){i[a(this).attr("value")]=a(this).text()});e.showOnFocus&&B.focus(function(){d.show()});e.showOnTap&&d.tap(B,function(){d.show()});l=j.val()||[];r=0;for(r;r<l.length;r++)d._selectedValues[l[r]]=l[r];A(i[s]);j.off(".dwsel").on("change.dwsel",function(){b||d.setValue(t?j.val()||[]:[j.val()],
