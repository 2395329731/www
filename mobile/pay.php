...
isset($auth) or $auth = '';
$_auth = decrypt($auth, AJ_KEY.'PAY');
$_auth or dheader('channel.php?reload='.$AJ_TIME);
list($moduleid, $itemid, $currency, $fee, $title) = explode('|', $_auth);
isset($MODULE[$moduleid]) or dheader('channel.php?reload='.$AJ_TIME);
$itemid = intval($itemid);
$itemid or dheader('channel.php?reload='.$AJ_TIME);
$fee = dround($fee);
$fee > 0 or dheader('channel.php?reload='.$AJ_TIME);
...
if($currency == 'money') {
	if(isset($password)) {
		...
		$db->query("INSERT INTO {$AJ_PRE}finance_pay (moduleid,itemid,username,fee,currency,paytime,ip,title) VALUES ('$moduleid','$itemid','$_username','$fee','$currency','$AJ_TIME','$AJ_IP','".addslashes($title)."')");
		...
		$head_title = $L['pay_title'].$AJ['seo_delimiter'].$head_title;
		$foot = 'channel';
	...
	dheader('channel.php?reload='.$AJ_TIME);
}
?>