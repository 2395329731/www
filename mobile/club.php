...
require AJ_ROOT.'/module/club/common.inc.php';
$gid = isset($gid) ? intval($gid) : 0;
if($gid) {
	$GRP = get_group($gid);
	($GRP && $GRP['status'] == 3) or mobile_msg($L['my_not_group']);
	$head_title = $GRP['title'].$MOD['seo_name'].$AJ['seo_delimiter'].$MOD['name'].$AJ['seo_delimiter'].$head_title;
}
...
switch($action) {
	...
	case 'fans_join':
		...
		$reason = convert(input_trim($reason), 'UTF-8', AJ_CHARSET);
		$reason = dhtmlspecialchars($reason);
		if(strlen($reason) > AJ_CHARLEN*500) exit('ko');
		if($GRP['join_type']) {
			if(strlen($reason) < 3) exit('reason');
		}
		$status = $GRP['join_type'] ? 2 : 3;
		$db->query("INSERT INTO {$table}_fans (gid,username,passport,reason,addtime,status) VALUES ('$gid','$_username','$_passport','$reason','$AJ_TIME','$status')");
	...
	case 'fans':
		...
		$M = $db->get_one("SELECT * FROM {$table}_fans WHERE gid=$gid AND username='$_username'");
		$is_fans = $M ? 1 : 0;
		$head_title = $L['fans_title'].$AJ['seo_delimiter'].$head_title;
		$foot = '';
	...
	case 'reply':
		...
		if(isset($_POST['ok'])) {
			...
			if($msg) exit('captcha');
			$post = array();
			$post['content'] = isset($content) ? convert(input_trim(nl2br($content)), 'UTF-8', AJ_CHARSET) : '';
		...
		$head_title = $L['reply_title'].$AJ['seo_delimiter'].$head_title;
		$foot = '';
	...
	case 'post':
		...
		if($fee_add) mobile_msg($L['post_msg_fee'].'<a href="'.$MODULE[2]['linkurl'].$AJ['file_my'].'?mid='.$moduleid.'&action=add&gid='.$gid.'" rel="external" class="b">'.$L['post_msg_advance'].'</a>');
		...
		if(isset($_POST['ok'])) {
			...
			$post['title'] = isset($title) ? convert(input_trim($title), 'UTF-8', AJ_CHARSET) : '';
			$post['content'] = isset($content) ? convert(input_trim(nl2br($content)), 'UTF-8', AJ_CHARSET) : '';
			require AJ_ROOT.'/include/post.func.php';
		...
		$head_title = $L['post_title'].$AJ['seo_delimiter'].$head_title;
		$foot = '';
