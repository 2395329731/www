...
$addr_url = 'address.php?auth='.encrypt($AJ_URL, AJ_KEY.'ADDR');
if($addr_id) {
	$t = $db->get_one("SELECT * FROM {$AJ_PRE}address WHERE itemid=$addr_id");
	if($t && $t['username'] == $_username) $addr = $t;
}
if(!$addr) $addr = $db->get_one("SELECT * FROM {$AJ_PRE}address WHERE username='$_username' ORDER BY listorder ASC,itemid ASC");
...
switch($module) {
	case 'mall':
		...
		if(isset($_POST['ok'])) {
			...
			$amount = $number*$price;			
			$_note = convert(input_trim($note), 'UTF-8', AJ_CHARSET);
			$note = '';
			...
			if($express && in_array($express, array(1,2,3))) {
				...
				if($express_id && $area_id) {
					$E = $db->get_one("SELECT * FROM {$AJ_PRE}mall_express WHERE itemid=$express_id");
					if($E && $E['items'] > 0) {
						$AREA = cache_read('area.php');
						$aid = $area_id;
						$ii = 0;
						do {
							$E = $db->get_one("SELECT * FROM {$AJ_PRE}mall_express WHERE parentid=$express_id AND areaid=$aid");
							if($E) {
			...
			$db->query("INSERT INTO {$AJ_PRE}mall_order (mid,mallid,buyer,seller,title,thumb,price,number,amount,addtime,updatetime,note, buyer_postcode,buyer_address,buyer_name,buyer_phone,buyer_mobile,status,fee_name,fee,cod) VALUES ('$moduleid','$itemid','$_username','$t[username]','$title','$t[thumb]','$price','$number','$amount','$AJ_TIME','$AJ_TIME','$note','$buyer_postcode','$buyer_address','$buyer_name','$buyer_phone','$buyer_mobile','$status','$fee_name','$fee','$cod')");
			$oid = $db->insert_id();
			$db->query("REPLACE INTO {$AJ_PRE}mall_comment (itemid,mallid,buyer,seller) VALUES ('$oid','$itemid','$_username','$t[username]')");
			$tmp = $db->get_one("SELECT mallid FROM {$AJ_PRE}mall_stat WHERE mallid=$itemid");
			if(!$tmp) $db->query("REPLACE INTO {$AJ_PRE}mall_stat (mallid,buyer,seller) VALUES ('$itemid','$_username','$t[username]')");
			$touser = $t['username'];
			$_title = $title;
			...
			$content = lang($L['trade_message_c6'], array(userurl($_username), $_username, timetodate($AJ_TIME, 3), $goods, $oid, $amount, $url));
	...
	case 'group':
		...
		if(isset($_POST['ok'])) {
			...
			$price = $item['price'];
			$amount = $number*$price;
			$note = convert(input_trim($note), 'UTF-8', AJ_CHARSET);
			$note = dhtmlspecialchars($note);
			$title = addslashes($item['title']);			
			$db->query("INSERT INTO {$AJ_PRE}group_order (gid,buyer,seller,title,thumb,price,number,amount,logistic,addtime,updatetime,note, buyer_postcode,buyer_address,buyer_name,buyer_phone,buyer_mobile,status) VALUES ('$itemid','$_username','$item[username]','$title','$item[thumb]','$item[price]','$number','$amount','$item[logistic]','$AJ_TIME','$AJ_TIME','$note','$buyer_postcode','$buyer_address','$buyer_name','$buyer_phone','$buyer_mobile', 6)");
			$oid = $db->insert_id();
			exit('ok|'.$oid);
		}
	break;
	case 'sell':
		...
		if(isset($_POST['ok'])) {
			...
			$fee_name = '';
			$fee = $cod = 0;
			$db->query("INSERT INTO {$AJ_PRE}mall_order (mid,mallid,buyer,seller,title,thumb,price,number,amount,addtime,updatetime,note, buyer_postcode,buyer_address,buyer_name,buyer_phone,buyer_mobile,status,fee_name,fee,cod) VALUES ('$moduleid','$itemid','$_username','$item[username]','$title','$item[thumb]','$price','$number','$amount','$AJ_TIME','$AJ_TIME','$note','$buyer_postcode','$buyer_address','$buyer_name','$buyer_phone','$buyer_mobile','$status','$fee_name','$fee','$cod')");
			$oid = $db->insert_id();
			//send message
