...
mobile_login();
$MG['article_limit'] > -1 or dlayerm(lang('message->without_permission_and_upgrade'), 'goback');
...
switch($action) {
	case 'add':
		...
		if($MG['article_limit'] && $limit_used >= $MG['article_limit']) dlayerm(lang($L['info_limit'], array($MG[$MOD['module'].'_limit'], $limit_used)), $_userid ? $MODULE[2]['linkurl'].$AJ['file_my'].'?mid='.$mid : $MODULE[2]['linkurl'].$AJ['file_my']);
		if($MG['day_limit']) {
			$today = $today_endtime - 86400;
			$r = $db->get_one("SELECT COUNT(*) AS num FROM {$table} WHERE $sql AND addtime>$today");
			if($r && $r['num'] >= $MG['day_limit']) dlayerm(lang($L['day_limit'], array($MG['day_limit'])), $_userid ? $MODULE[2]['linkurl'].$AJ['file_my'].'?mid='.$mid : $MODULE[2]['linkurl'].$AJ['file_my']);
		}
		...
		$thumbs= explode(",", $thumbs);
		 ...
		 $post['thumb2'] = $thumbs[2];
			...
			if($MG['add_limit']) {
				...
				if($last && $AJ_TIME - $last['addtime'] < $MG['add_limit']) dtexit(lang($L['add_limit'], array($MG['add_limit'])));
			}
	...
	case 'edit':
		...
		if($MG['edit_limit'] && $AJ_TIME - $item['addtime'] > $MG['edit_limit']*86400) dlayerm(lang($L['edit_limit'], array($MG['edit_limit'])));
