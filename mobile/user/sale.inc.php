...
switch($action) {
	case 'add':
		...
		if($MG['sale_free_limit'] >= 0) {
			$fee_add = ($MOD['fee_add'] && (!$MOD['fee_mode'] || !$MG['fee_mode']) && $limit_used >= $MG['sale_free_limit'] && $_userid) ? dround($MOD['fee_add']) : 0;
		} else {
			$fee_add = 0;
		}
		...
		$thumbs= explode(",", $thumbs);
		 ...
		 $post['thumb2'] = $thumbs[2];
			...
			if($do->pass($post)) {
				...
				if($MOD['upload_thumb'] && $_userid && strlen($post['thumb']) < 5) dtexit($L['sale_upload_image']);
			    ...
			    $db->query("UPDATE {$AJ_PRE}sale_5 SET note='$note' WHERE itemid='$do->itemid'");
				 if($thumbs) {
			...
			$item = array();
			$mycatid_select = type_select('product-'.$_userid, 0, 'post[mycatid]', $L['sale_default_type']);
		}
	break;
	case 'edit':
		...
		if(isset($_POST['ok'])) {
		 ...
		 $post['thumb2'] = $thumbs[2];
			...
			if($do->pass($post)) {
				...
				foreach($thumbs as $k=>$v) {
					if($v) {
						...
						if(empty($thumb)) {$db->query("INSERT INTO {$AJ_PRE}house_pic (item,thumb,mid) VALUES ('$do->itemid', '$v',5)");}
			...
			$mycatid_select = type_select('product-'.$_userid, 0, 'post[mycatid]', $L['sale_default_type'], $mycatid);
		}
	break;
	case 'delete':
