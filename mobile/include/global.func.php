...
function mobile_login() {
	global $_userid, $AJ_URL;
	$_userid or dheader('login.php?forward='.rawurlencode($AJ_URL));
}
...
function toutf8() {
	...
	ob_end_clean();
	echo convert($data, AJ_CHARSET, 'UTF-8');
}
...
function is_pc() {
	...
	if(strpos($UA, 'WINDOWS NT') !== false) {
		global $AJ_URL;
		if(strpos($AJ_URL, 'plg_') !== false) return false;//QQ
		return true;
	}
	return false;
}
