...
if($itemid) {
	...
	if($submit) {
		...
		$r = $db->get_one("SELECT rid FROM {$AJ_PRE}form_record WHERE fid=$itemid $condition");
		if($r) message($L['form_repeat']);
		if($fromtime && $AJ_TIME < $fromtime) $could_form = false;
		if($totime && $AJ_TIME > $totime) $could_form = false;
	...
	//$display = 0;
	$db->query("UPDATE {$AJ_PRE}form SET hits=hits+1 WHERE itemid=$itemid");
