...
class spread {
	...
	function set($post) {
		global $MOD, $AJ_TIME, $_username, $_userid, $AJ_PRE;
		...
		$m = $this->db->get_one("SELECT company FROM {$AJ_PRE}member WHERE username='$post[username]'");
	...
	function _add($post) {
		...
		$this->db->query("INSERT INTO {$this->table_price} (word,sell_price,buy_price,company_price,editor,edittime) VALUES('$post[word]','$post[sell_price]','$post[buy_price]','$post[company_price]','$_username','$AJ_TIME')");
	}
