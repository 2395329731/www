...
if(in_array($itemid, cache_read('bancomment-'.$mid.'.php'))) {
	...
	include template('commentlp', $module);
	exit;
}
if($mid == 4) {
...
switch($action) {
	...
	case 'delete':
		...
		if($r) {
			...
			$forward = $MOD['linkurl'].'commentlp.php?mid='.$mid.'&itemid='.$itemid.'&page='.$page.'&rand='.mt_rand(10, 99);
	...
	default:
		...
		if($submit) {
			...
			$db->query("INSERT INTO {$AJ_PRE}comment (item_mid,item_id,item_title,item_username,content,quotation,qid,addtime,username,hidden,star,ip,status) VALUES ('$mid','$itemid','".addslashes($title)."','$username','$content','$quotation','$qid','$AJ_TIME','$_username','$hidden','$star','$AJ_IP','$status')");
			$cid = $db->insert_id();
			...
			if($status == 3) {
				...
				if($MOD['comment_show']) {
					$forward = $MOD['linkurl'].'commentlp.php?mid='.$mid.'&itemid='.$itemid.'&page='.$page.'&rand='.mt_rand(10, 99).'#last';
