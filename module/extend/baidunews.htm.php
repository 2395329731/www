...
foreach($news as $v) {
	...
	$result = $db->query("SELECT * FROM {$AJ_PRE}article_{$mid} a,{$AJ_PRE}article_data_{$mid} d WHERE a.itemid=d.itemid AND a.status=3 ORDER BY a.addtime DESC LIMIT $news_num");
