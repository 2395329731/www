...
class photo {
	...
	function update($itemid) {
		...
		$update = '';
		$keyword = $item['housename'].','.$item['title'].','.strip_tags(cat_pos(get_cat($item['catid']), ','));
	...
	function item_update($post) {
		...
		if($post[0]['thumb']) {
			...
			$mianji = $post[0]['mianji'];
			if($introduce == $L['photo_intro']) $introduce = '';
				$this->db->query("INSERT INTO {$this->table_item} (item,thumb,introduce,mianji,listorder) VALUES ('$this->itemid', '$thumb','$introduce','$mianji','$listorder')");
		}
		unset($post[0]);
		foreach($post as $k=>$v) {
			if(isset($v['delete'])) {
			...
			if($v['thumb']) {				
				...
				$mianji = $v['mianji'];
				if($introduce == $L['photo_intro']) $introduce = '';
				$this->db->query("UPDATE {$this->table_item} SET thumb='$thumb',introduce='$introduce',mianji='$mianji',listorder='$listorder' WHERE itemid=$k");
			} else {
				$this->item_delete($k);
			}
		}
