...
function addcaiji($post) {
	...
	function renttop($itemid,$to_time) {
	       $rent=$this->db->pre.'rent_7';
			$this->db->query("UPDATE $rent SET istop=1,to_time=$to_time WHERE itemid=$itemid");
			return true;
          	}
	function renthot($itemid,$to_time) {
	       $rent=$this->db->pre.'rent_7';
			$this->db->query("UPDATE $rent SET ishot=1,hot_time=$to_time WHERE itemid=$itemid");
			return true;
     }
