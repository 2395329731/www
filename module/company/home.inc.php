...
switch($action) {
	case 'search':
		if(check_name($homepage) && in_array($file, array('sale', 'rent', 'news', 'newhouse','credit', 'job', 'photo', 'info',  'video')) && $kw) {
	...
	case 'next':
		...
		$domain = $user['domain'];
		if($domain) $AJ['rewrite'] = intval($CFG['com_rewrite']);
		$r = $db->get_one("SELECT itemid FROM {$AJ_PRE}sale_5 WHERE username='$username' AND itemid>$itemid AND status=3 ORDER BY itemid ASC");
		if($r) dheader(userurl($username, 'file=sale&itemid='.$r['itemid'], $domain));
		dheader(userurl($username, 'file=sale', $domain));
	break;
	case 'prev':
		$itemid or dheader($MOD['linkurl']);
		check_name($username) or dheader($MOD['linkurl']);
		...
		$domain = $user['domain'];
		if($domain) $AJ['rewrite'] = intval($CFG['com_rewrite']);
		$r = $db->get_one("SELECT itemid FROM {$AJ_PRE}sale_5 WHERE username='$username' AND itemid<$itemid AND status=3 ORDER BY itemid DESC");
		if($r) dheader(userurl($username, 'file=sale&itemid='.$r['itemid'], $domain));
		dheader(userurl($username, 'file=sale', $domain));
	break;
	default:
