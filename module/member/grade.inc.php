...
if($_userid) {
	...
	$r = $db->get_one("SELECT addtime FROM {$AJ_PRE}upgrade WHERE ip='$AJ_IP' ORDER BY itemid DESC");
	if($r && $AJ_TIME - $r['addtime'] < 86400) $could_up = false;
}
if($submit && $could_up) {
	...
	if($fee) {
		if($promo_code) {
			$p = $db->get_one("SELECT * FROM {$AJ_PRE}finance_promo WHERE number='$promo_code' AND totime>$AJ_TIME");
			if($p && ($p['reuse'] || (!$p['reuse'] && !$p['username']))) {
				$promo_type = $p['type'];
		...
		if($promo_code) {
			...
			$db->query("UPDATE {$AJ_PRE}finance_promo SET username='$_username',ip='$AJ_IP',".($p['reuse'] ? "updatetime=updatetime+1" : "updatetime='$AJ_TIME'")." WHERE number='$promo_code'");
		} else {
			if($_money > $fee) {
				$amount = $fee;
				money_add($_username, -$amount);
	...
	$db->query("INSERT INTO {$AJ_PRE}upgrade (userid,username,groupid,company,truename,telephone,mobile,email,msn,qq,ali,skype,content,addtime,ip,amount,promo_code,promo_type,promo_amount,status) VALUES ('$_userid','$_username', '$groupid','$company','$truename','$telephone','$mobile','$email','$msn','$qq','$ali','$skype','$content', '$AJ_TIME', '$AJ_IP','$amount','$promo_code','$promo_type','$promo_amount','2')");
	 message($L['grade_msg_success'], AJ_PATH, 5);
} else {
	$GROUPS = array();
	foreach($GROUP as $k=>$v) {
		if($k > 4) {
