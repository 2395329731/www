...
if($action == 'update') {
	...
	$td['linkurl'] = AJ_PATH.'api/redirect.php?mid=17&itemid='.$td['gid'];
	$gid = $td['gid'];
	...
	switch($step) {
		...
		case 'pay'://买家付款
			...
			if($submit) {
				...
				$db->query("UPDATE {$AJ_PRE}group SET orders=orders+1,sales=sales+$td[number] WHERE itemid=$td[gid]");
	...
	while($r = $db->fetch_array($result)) {
		...
		$r['linkurl'] = AJ_PATH.'api/redirect.php?mid=17&itemid='.$r['gid'];
