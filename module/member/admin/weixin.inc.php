...
switch($action) {
	...
	case 'user':
		...
		$lists = array();
		$result = $db->query("SELECT * FROM {$AJ_PRE}weixin_user WHERE $condition ORDER BY $order LIMIT $offset,$pagesize");
		while($r = $db->fetch_array($result)) {
	...
	case 'menu':
		if($submit) {
			...
			for($i = 0; $i < 3; $i++) {
				if($post[$i][0]['name']) {
					$menu[$i]['name'] = urlencode(convert($post[$i][0]['name'], AJ_CHARSET, 'UTF-8'));
					if($sub[$i]) {
						for($j = 1; $j < 6; $j++) {
							if($post[$i][$j]['name'] && $post[$i][$j]['key']) {
									$menu[$i]['sub_button'][$j-1]['name'] = urlencode(convert($post[$i][$j]['name'], AJ_CHARSET, 'UTF-8'));
	...
	default:
		...
		$pages = pages($items, $page, $pagesize);
		$lists = array();
		$result = $db->query("SELECT u.username,u.nickname,u.sex,u.city,u.province,u.country,u.language,u.headimgurl,c.* FROM {$AJ_PRE}weixin_chat c,{$AJ_PRE}weixin_user u WHERE $condition ORDER BY c.addtime DESC LIMIT $offset,$pagesize");
		while($r = $db->fetch_array($result)) {
			$r['adddate'] = timetodate($r['addtime'], 5);
			$r['headimgurl'] or $r['headimgurl'] = 'api/weixin/image/headimg.jpg';
