...
switch($action) {
	case 'edit':
		$itemid or msg();
		$a = $db->get_one("SELECT * FROM {$AJ_PRE}ask WHERE itemid=$itemid");
		$a or msg();
		if($submit) {
			if($status > 1 && strlen($reply) < 5) msg('请填写回复内容');
			$reply = addslashes(save_remote(save_local(stripslashes($reply))));
			$db->query("UPDATE {$AJ_PRE}ask SET status=$status,editor='$_username',edittime='$AJ_TIME',reply='$reply' WHERE itemid=$itemid");
			if($status > 1) {
				$msg = isset($msg) ? 1 : 0;
				$eml = isset($eml) ? 1 : 0;
				$sms = isset($sms) ? 1 : 0;
			...
			if($status == 0) {
				$status = 1;
				$db->query("UPDATE {$AJ_PRE}ask SET status=1,edittime=$AJ_TIME WHERE itemid=$itemid");
			}
