...
class member {
	...
	function is_member($member) {
		...
		if(AJ_MAX_LEN && strlen($member['content']) > AJ_MAX_LEN) return $this->_(lang('message->pass_max'));
		return true;
	}
	...
	function add($member) {
		...
		$member['sound'] = 1;
		$member['letter'] = GetPinyin($member['company']);
		$member_fields = array('username','company','passport', 'password','payword','email','sound','gender','truename','mobile','msn','qq','ali','skype','department','career','groupid','regid','areaid','edittime','inviter','passsalt', 'paysalt','companyid');
	...
	function edit($member)	{
		...
		$member = $this->set_member($member);
		$r = $this->get_one();
		$member_fields = array('company','passport','sound','email','msn','qq','ali','skype','gender','truename','mobile','department','career','groupid','areaid', 'edittime','black','bank','banktype','branch','account','vemail','vmobile','vbank','vtruename','vcompany','vtrade','trade','support','inviter','companyid');
	...
	function login($login_username, $login_password, $login_cookietime = 0, $admin = false) {
		...
		$auth = encrypt($user['userid'].'|'.$user['password'], AJ_KEY.'USER');
	...
	function delete($userid) {
		...
		while($r = $this->db->fetch_array($result)) {
			...
			foreach(array('address', 'admin_log', 'alert', 'ask','comment', 'honor', 'finance_card', 'finance_cash', 'finance_charge', 'finance_credit', 'finance_deposit', 'finance_pay', 'finance_record', 'finance_sms', 'form_answer', 'gift_order', 'guestbook',  'link', 'login', 'mail_list', 'spread', 'upgrade', 'validate', 'news', 'oauth', 'page', 'poll_record', 'vote_record', 'weixin_bind', 'weixin_user') as $v) {
	...
	function rename($cusername, $nusername) {
		...
		foreach($MODULE as $m) {
			if($m['islink'] || $m['moduleid'] < 5) continue;
			if($m['moduleid']!=18)$tables[] = in_array($m['module'], array('article', 'sale', 'buy', 'rent', 'info', 'photo', 'newhouse', 'video')) ? $m['module'].'_'.$m['moduleid'] : $m['module'];
		}
	...
	function login_log($username, $password, $salt, $admin = 0, $message = '') {
		...
		$this->db->query("INSERT INTO {$this->db->pre}login (username,password,passsalt,admin,loginip,logintime,message,agent) VALUES ('$username','$password','$salt','$admin','$AJ_IP','$AJ_TIME','$message','$agent')");
	}
	...
	function check_add($post) {
		...
		$this->db->query("REPLACE INTO {$this->table_member_check} (userid,username,content,addtime) VALUES ('$this->userid','$this->username','$content','$AJ_TIME')");
	}
