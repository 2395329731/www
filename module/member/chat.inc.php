...
login();
$AJ['im_web'] or dheader($MOD['linkurl']);
...
switch($action) {
	case 'send':		
		...
		if(is_file($filename)) {
			if(filesize($filename) > 500*1024) {
				file_copy($filename, substr($filename, 0, -4).'-'.timetodate($AJ_TIME, 'YmdHis').'.php');
		...
		if($word && $fp = fopen($filename, 'a')) {
			fwrite($fp, $AJ_TIME.'|'.$_username.'|'.$word."\n");
			fclose($fp);
			exit('ok');
		}
		exit('ko');
	...
	case 'down':
		if($data && check_name($username) && is_md5($chatid)) {
			...
			$data = '<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd"><html xmlns="http://www.w3.org/1999/xhtml"><head><meta http-equiv="Content-Type" content="text/html;charset='.AJ_CHARSET.'"/><title>'.lang($L['chat_record'], array($username)).'</title><style type="text/css">'.$css.'</style><base href="'.$MOD['linkurl'].'"/></head><body><div id="chat">'.$data.'</div></body></html>';
			file_down('', 'chat-'.$username.'-'.timetodate($AJ_TIME, 'Y-m-d-H-i').'.html', $data);
		}
		exit;
	break;
	case 'list':
