...
$member = $db->get_one("SELECT company,truename,vbank,money,bank,banktype,branch,account FROM {$AJ_PRE}member WHERE userid=$_userid");
...
switch($action) {
	...
	case 'setting':
		...
		if($submit && !$vbank) {
			...
			$db->query("UPDATE {$AJ_PRE}member SET bank='$bank',banktype='$banktype',branch='$branch',account='$account' WHERE username='$_username' ");
	...
	case 'confirm':
		...
		if($MOD['cash_times']) {
			$r = $db->get_one("SELECT COUNT(*) as num FROM {$AJ_PRE}finance_cash WHERE username='$_username' AND addtime>$AJ_TIME-3600*24");
