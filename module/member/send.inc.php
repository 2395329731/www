...
switch($action) {
	case 'check':
		...
		$verify = 1;
		if(isset($auth)) {
			$auth = decrypt($auth, AJ_KEY.'REG');
			if($auth) {
				list($umail, $utime) = explode('|', $auth);
				if(is_email($umail) && $AJ_TIME - $utime < $MOD['auth_days']*600) {
					$step = 1;
					$verify = 0;
					$email = $umail;
					$captcha = '';
		...
		if($step == 2) {
			...
			$r = $db->get_one("SELECT userid,username,groupid,regid FROM {$AJ_PRE}member WHERE email='$email'");
			if($r && $r['groupid'] == 4) {
				$userid = $r['userid'];
				$username = $r['username'];
				$groupid = $r['regid'];
				$groupid > 4 or $groupid = 5;
				$db->query("UPDATE {$AJ_PRE}member SET groupid='$groupid',vemail=1 WHERE userid='$userid'");
			...
			$r = $db->get_one("SELECT groupid FROM {$AJ_PRE}member WHERE email='$email'");
			if($r) {
	...
	case 'payword':
		...
		if($step == 2) {
			...
			$db->query("UPDATE {$AJ_PRE}member SET payword='$pass',paysalt='$salt',vemail=1 WHERE userid=$_userid");
			userclean($username);
			unset($_SESSION['email_save']);
			unset($_SESSION['email_code']);
		} else if($step == 1) {
	...
	case 'mobile':
		...
		$session = new dsession();
		$t = $db->get_one("SELECT mobile FROM {$AJ_PRE}member WHERE userid=$_userid");
		$_mobile = $t['mobile'];
		if($step == 2) {
	...
	case 'contact':
		$url = AJ_PATH;
		if(is_file(AJ_ROOT.'/about/contact.html')) {
			$url = AJ_PATH.'about/contact.html';
		} else if(is_file(AJ_ROOT.'/about/index.html')) {			
			$url = AJ_PATH.'about/index.html';
		}
	...
	case 'sms':
		...
		if($step == 2) {
			...
			$r = $db->get_one("SELECT userid,username,groupid,vmobile FROM {$AJ_PRE}member WHERE mobile='$mobile' AND vmobile=1");
			if($r) {
			...
			$mobile = trim($mobile);
			if(!is_mobile($mobile)) message($L['member_mobile_null']);
			$r = $db->get_one("SELECT groupid FROM {$AJ_PRE}member WHERE mobile='$mobile' AND vmobile=1");
			if($r) {
				if($r['groupid'] == 2 || $r['groupid'] == 4) message($L['send_password_checking']);
				$mobilecode = random(6, '0123456789');
				$_SESSION['mobile_save'] = $mobile;
	...
	case 'mail':
		...
		if($step == 2) {
			...
			$r = $db->get_one("SELECT userid,username,groupid FROM {$AJ_PRE}member WHERE email='$email'");
			if($r) {
				if($r['groupid'] == 2 || $r['groupid'] == 4) message($L['send_password_checking']);
				$userid = $r['userid'];
				$username = $r['username'];
				$salt = random(8);
				$pass = dpassword($password, $salt);
				$db->query("UPDATE {$AJ_PRE}member SET password='$pass',passsalt='$salt',vemail=1 WHERE userid='$userid'");
				unset($_SESSION['email_save']);
				unset($_SESSION['email_code']);
