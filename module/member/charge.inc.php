...
function get_reason_url($r = array()) {
	...
	switch($arr[0]) {
		...
		break;
		case 'pay':
			if(is_numeric($arr[1]) && is_numeric($arr[2])) $url = AJ_PATH.'api/redirect.php?mid='.intval($arr[1]).'&itemid='.intval($arr[2]).'&sum=1';
		break;
		case 'trade':
			if(is_numeric($arr[1])) $url = 'trade.php?action=update&step=pay&itemid='.intval($arr[1]).'&auth='.encrypt($r['amount'], AJ_KEY.'CG', 600);
		break;
		case 'trades':
			$url = 'trade.php?action=muti&step=pay&auth='.encrypt($arr[1], AJ_KEY.'CG', 600);
		break;
		case 'group':
			if(is_numeric($arr[1])) $url = 'group.php?action=update&step=pay&itemid='.intval($arr[1]).'&auth='.encrypt($r['amount'], AJ_KEY.'CG', 600);
		break;
		default:
		break;
	}
