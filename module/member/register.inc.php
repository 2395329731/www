...
if($submit) {
	...
	if($do->add($post)) {
		...
		//send sms
		if($MOD['checkuser'] == 2) {
			$goto = 'send.php?action=check&auth='.encrypt($email.'|'.$AJ_TIME, AJ_KEY.'REG');
		...
		$forward = 'goto.php?action=register_success&username='.$username.'&auth='.encrypt('LOGIN|'.$username.'|'.$post['password'].'|'.$AJ_TIME, AJ_KEY.'LOGIN').'&forward='.urlencode($forward);
