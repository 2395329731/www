...
if($action == 'update') {
	...
	$td['linkurl'] = AJ_PATH.'api/redirect.php?mid='.$td['mid'].'&itemid='.$td['mallid'];
	$td['par'] = '';
	...
	switch($step) {
		case 'edit_price'://修改价格||确认订单||修改为货到付款
			...
			if($AJ['trade'] && $_trade == '') message(lang($L['trade_msg_pay_bind'], array($AJ['trade_nm'])), '?action=bind');
			if($submit) {
				$fee = dround($fee);
		...
		case 'pay'://买家付款
			...
			$auto = 0;
			$auth = isset($auth) ? dround(decrypt($auth, AJ_KEY.'CG')) : '';
		...
		case 'refund_agree'://卖家同意买家退款
			...
			if($submit) {
				...
				$db->query("UPDATE {$table} SET status=6,editor='$_username',updatetime=$AJ_TIME,refund_reason='$content' WHERE itemid=$itemid");
				//更新商品数据 增加库存
				if($td['mid'] == 16) {
					$db->query("UPDATE {$AJ_PRE}mall SET orders=orders-1,sales=sales-$td[number],amount=amount+$td[number] WHERE itemid=$mallid");
				} else {
		...
		case 'send_goods'://卖家发货
			...
			if($AJ['trade'] && $td['status'] == 2) exit(include AJ_ROOT.'/api/trade/'.$AJ['trade'].'/update.inc.php');
			if($submit) {
				...
				$db->query("UPDATE {$table} SET status=$status,updatetime=$AJ_TIME,send_type='$send_type',send_no='$send_no',send_time='$send_time' WHERE itemid=$itemid");
	...
	$AJ['trade'] or message($L['trade_msg_secured_close']);
	...
	if($submit) {
		...
		if($trade) {
			if($AJ['trade'] == 'alipay' && !is_email($trade) && !is_mobile($trade)) message(lang($L['trade_bind_error'], array($AJ['trade_nm'])));
		...
		$head_title = lang($L['trade_bind_title'], array($AJ['trade_nm']));
	}
	...
	if($submit) {
		...
		while($r = $db->fetch_array($result)) {
			...
			$r['linkurl'] = AJ_PATH.'api/redirect.php?mid='.$r['mid'].'&itemid='.$r['mallid'];
