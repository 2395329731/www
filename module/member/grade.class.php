...
class grade {
	...
	function edit($post) {
		...
		$gsql = $msql = $csql = '';
		$gsql = "edittime=$AJ_TIME,editor='$_username',status=$post[status],note='$post[note]'";
		if($post['status'] == 1) {
			//reject
			if($user) {
		...
		if($msql) $this->db->query("UPDATE {$AJ_PRE}member SET $msql WHERE userid=$item[userid]");
		if($csql) $this->db->query("UPDATE {$AJ_PRE}company SET $csql WHERE userid=$item[userid]");
		return true;
	}
