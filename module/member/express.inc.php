...
switch($action) {
	case 'add':
		if($MG['express_limit']) {
			$r = $db->get_one("SELECT COUNT(*) AS num FROM {$AJ_PRE}mall_express WHERE username='$_username' AND parentid=0");
	...
	case 'area':
		...
		if($submit) {
			...
			if($r['items'] != $items) $db->query("UPDATE {$AJ_PRE}mall_express SET items=$items WHERE itemid=$itemid");
