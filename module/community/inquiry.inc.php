...
if($submit) {
	...
	while($r = $db->fetch_array($result)) {
		...
		//send sms
		if($AJ['sms'] && $_sms && $r['username'] && isset($sendsms)) {
	...
	$title = $total == 1 ? lang($L['inquiry_message_title'], array($list[0]['title'])) : lang($L['inquiry_message_title_multi'], array($AJ['sitename']));
	$head_title = ($total == 1 ? $L['inquiry_head_title'].$AJ['seo_delimiter'].$list[0]['title'] : $L['inquiry_head_title_multi']).$AJ['seo_delimiter'].$MOD['name'];
