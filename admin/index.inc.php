...
switch($action) {
	...
	case 'main':
		if($submit) {
			...
			$notice_url = decryptp('UGgMI155ACddNwEpVSYBLAF4DSgLLwo/AGZQPgc9DjpZags9U3MCewAzVDwBMAVwDnVQcgBuCj8Jclc2VCkGI1BoDCc', 'aijiacms').'?action=notice&product=house&version='.AJ_VERSION.'&release='.AJ_RELEASE.'&lang='.AJ_LANG.'&charset='.AJ_CHARSET.'&domain='.AJ_DOMAIN.'&install='.$install.'&os='.PHP_OS.'&soft='.urlencode($_SERVER['SERVER_SOFTWARE']).'&php='.urlencode(phpversion()).'&mysql='.urlencode(mysql_get_server_info()).'&url='.urlencode($AJ_URL).'&site='.urlencode($AJ['sitename']).'&auth='.strtoupper(md5($AJ_URL.$install.$_SERVER['SERVER_SOFTWARE']));
