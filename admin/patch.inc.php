...
if($action == 'view') {
	...
	if($submit) {
		is_date($fd) or $fd = timetodate($AJ_TIME, 3);
		if($fh < 0 || $fh > 23) $fh = 0;
		if($fm < 0 || $fm > 59) $fm = 0;
		$ft = strtotime($fd.' '.($fh > 9 ? $fh : '0'.$fh).':'.($fm > 9 ? $fm : '0'.$fm).':00');

		is_date($td) or $td = timetodate($AJ_TIME, 3);
		if($th < 0 || $th > 23) $th = 23;
		if($tm < 0 || $tm > 59) $tm = 59;
		...
		$fd = substr(AJ_RELEASE, 0, 4).'-'.substr(AJ_RELEASE, 4, 2).'-'.substr(AJ_RELEASE, 6, 2);
		$td = timetodate($AJ_TIME, 3);
