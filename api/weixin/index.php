		...
		if($MsgType == 'event') {//事件
			...
			switch($Event) {
				case 'CLICK'://点击菜单拉取消息时的事件推送
					switch($EventKey) {
						case 'V_member'://绑定会员
							...
							$tags['url'] = $EXT['mobile_url'].'weixin.php?action=member&auth='.encrypt("$FromUserName", AJ_KEY.'WXID');
							$misc[] = $tags;						
						...
						default:
							if(substr($EventKey, 0, 5) == 'V_mid') {
								...
								if(in_array($mod, array('article', 'group', 'video', 'photo'))) {
									$post['content'] = $EventKey;
									$misc = array();
								...
								} else if(in_array($mod, array('newhouse', 'sale', 'rent', 'buy', 'info'))) {
									$post['content'] = $EventKey;
									$misc = array();
				...
				case 'subscribe'://订阅
					...
					$sql = "subscribe=1,addtime=".$info['subscribe_time'].",edittime=$AJ_TIME,visittime=$AJ_TIME";
			...
			switch($MsgType) {
				case 'text'://文本消息
					$Content = convert("$x->Content", 'UTF-8', AJ_CHARSET);
				...
				case 'location'://地理位置消息
					$post['content'] = convert("$x->Label", 'UTF-8', AJ_CHARSET);
				...
				case 'link'://链接消息
					$post['content'] = $x->Url;
					$post['misc']['Title'] = convert("$x->Title", 'UTF-8', AJ_CHARSET);
					$post['misc']['Description'] = convert("$x->Description", 'UTF-8', AJ_CHARSET);
				break;
				default:
				break;
			}
