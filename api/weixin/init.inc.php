...
require AJ_ROOT.'/api/weixin/config.inc.php';
...
class weixin {
	...
	function get_user($openid) {
		...
		if(is_array($arr)) {
			foreach($arr as $k=>$v) {
				$arr[$k] = convert($v, 'UTF-8', AJ_CHARSET);
			}
		}
		return $arr;
	}
	...
	function send($openid, $type, $content, $misc = array()) {
		...
		switch($type) {
			case 'text':
				$par[$type]['content'] = urlencode(convert($content, AJ_CHARSET, 'UTF-8'));
			break;
			case 'image':
			...
			case 'video':
				$par[$type]['media_id'] = $content;
				$par[$type]['title'] = isset($misc['title']) ? urlencode(convert($misc['title'], AJ_CHARSET, 'UTF-8')) : '';
				$par[$type]['description'] = isset($misc['description']) ? urlencode(convert($misc['description'], AJ_CHARSET, 'UTF-8')) : '';
			break;
			case 'music':
				$par[$type]['title'] = isset($misc['title']) ? urlencode(convert($misc['title'], AJ_CHARSET, 'UTF-8')) : '';
				$par[$type]['description'] = isset($misc['description']) ? urlencode(convert($misc['description'], AJ_CHARSET, 'UTF-8')) : '';
			...
			case 'news':
				if($misc && count($misc) < 11) {
					foreach($misc as $k=>$v) {
						$misc[$k]['title'] = urlencode(convert($v['title'], AJ_CHARSET, 'UTF-8'));
						$misc[$k]['description'] = urlencode(convert($v['description'], AJ_CHARSET, 'UTF-8'));
					}
	...
	function response($openid, $from, $type, $content, $misc = array()) {
		...
		$xml .= '</xml>';
		echo convert($xml, AJ_CHARSET, 'UTF-8');
	}
}
$wx = new weixin;
